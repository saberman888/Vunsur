...
Status ScriptAccess::getTrending() {
	if( this->isLoggedIn() ) {
		return get_trending(this->acd);
	} else {
		Status s;
		not_logged_in(s); return s;
	}
...
Status ScriptAccess::deleteComment(std::string fullname)
{
	...
	if( this->isLoggedIn() )
	{
		return delete_comment(this->acd, fullname);
	} else {
		not_logged_in(s);
		return s;
	}
...
Status ScriptAccess::editComment(std::string id, std::string text)
{
	Status s;
	if( this->isLoggedIn() )
	{
		return edit_user_comment(this->acd, text, id);
	} else {
		not_logged_in(s);
		return s;
	}
